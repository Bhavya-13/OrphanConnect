  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      ...
      <p className="text-gray-500 mb-8 text-sm">
        Review each submission, its documents, and what it has requested — then approve or reject.
      </p>
          ...
          {orphanages.map((o) => {
            ...
            return (
              <div key={o.id} className="border border-orange-100 rounded-xl p-6 bg-white">
                ...
                <div className="bg-orange-50/60 border border-orange-100 rounded-lg p-3 mb-4">
                  <p className="text-xs font-semibold text-gray-600 mb-2">
                    Requested so far (review for anything suspicious)
                  </p>
                  ...
                  {orphNeeds.length === 0 && orphRequests.length === 0 ? (
                    ...
                    <>
                      {orphNeeds.length > 0 && (
                        <div className="mb-2">
                          <p className="text-xs font-medium text-gray-700">
                            Needs {totalMoneyAsked > 0 && `— total money asked: Rs ${totalMoneyAsked.toLocaleString()}`}
                          </p>
                          <ul className="text-xs text-gray-600 list-disc ml-5">
                            {orphNeeds.map((n) => (
                              <li key={n.id}>
                                {n.title} — {n.type === "money" ? `Rs ${n.amount_needed?.toLocaleString()}` : `${n.quantity_needed} ${n.unit}`}
                      ...
                      {orphRequests.length > 0 && (
                        <div>
                          <p className="text-xs font-medium text-gray-700">Volunteer requests</p>
                          <ul className="text-xs text-gray-600 list-disc ml-5">
                            {orphRequests.map((r) => (
                              <li key={r.id}>
                                {r.task} — {formatDate(r.date)} — {r.slots_available} slots
                              </li>
                            ))}
                          </ul>
                        </div>
